...
```
let sortedCitiesOne = cities.sorted(by: {a,b in
    return a.dropFirst() < b.dropFirst()
})
print(sortedCitiesOne)
```
c. Use `sortedBy` to sort `cities` in order of the length of the city name.
```
let sortedCitiesTwo = cities.sorted(by: {a,b in
    return a.count < b.count
})
print(sortedCitiesTwo)
```
## Question 6
...
```
let sortedCitiesWithPopulation = citiesWithPopulation.sorted(by: { x, y in
    x.1 < y.1
...
```let filter = numbers.filter({a -> Bool in
return a % 2 != 0
})
print(filter)
```
